...
class TestUpgradeLock:
    ...
    # cluster needing upgrade, and able to (no locked out)
    ...
    @patch.object(ous, "croniter", Mock(wraps=croniter))
    @patch.object(ous, "version_conditions_met", Mock())
    # cluster needing upgrade, but lock held by an other cluster
    ...
    @patch.object(ous, "croniter", Mock(wraps=croniter))
    @patch.object(ous, "version_conditions_met", Mock())
    # 2 clusters needing upgrade, but using the same lock. Only the first one will get upgraded
